...
// rophim-13 | Filter Series — v1
// Contract: filterSeries(baseUrl, sortIdx, sortVal, filterId1, filterIdx1, filterVal1, ..., page) -> JSON array
// API: GET https://k8s.onflixcdn.com/api/movies?type=phim-bo&page={page}&country={slug}&category={slug}&lang={lang}&year={year}&sort={sort}
//
// RC config stores DISPLAY NAMES in filter_list.values[].
// This resolver maps display names → API slugs internally.
//
// The app calls: filterSeries(baseUrl, sortIdx, sortVal, 'country', cIdx, cVal, 'category', gIdx, gVal, 'year', yIdx, yVal, 'lang', lIdx, lVal, page)
...
async function filterSeries(baseUrl) {
    ...
    try {
        ...
        // Always phim-bo for series
        if (filters['type']) {
            params.set('type', filters['type']);
        } else {
            params.set('type', 'phim-bo');
        }
