...
async function getMovieDetail(filmUrl) {
  ...
  function parseMoviePayload(html) {
    ...
    try {
      ...
      console.warn(`[KENG][RoPhim12] Failed to parse serialized movie payload: ${e.message}`);
      return null;
    }
  }

  try {
    console.log(`[KENG][RoPhim12] getMovieDetail: ${filmUrl}`);
    ...
    if (movieId) {
      try {
        ...
        console.warn(`[KENG][RoPhim12] Failed to fetch episodes: ${e.message}`);
      }
    }
    ...
    console.log(`[KENG][RoPhim12] getMovieDetail: ${detail.title} (${detail.media_type}) with ${parts.length} parts, ${totalEpisodes} episodes, ${actorItems.length} actors (${actorUrls} urls)`);
    return JSON.stringify(detail);

  } catch (e) {
    console.error(`[KENG][RoPhim12] getMovieDetail ERROR: ${e.message}`);
