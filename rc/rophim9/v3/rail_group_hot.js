...
/**
 * Dedicated Rail Group for Phim Hot
 * Fetch from: /baseapi/api/v1/movies/hot on the active provider origin
 * v6 architecture: Dedicated JS for unique API source
 */
...
async function railGroupHot(baseUrl) {
  ...
  console.log('[KENG][RoPhim10] railGroupHot() v6 — Dedicated API');
  ...
  try {
    const hotUrl = BASE_API + '/movies/hot';
    const hotRes = await fetch(hotUrl, buildFetchOptions(SITE_BASE + '/'));
    if (!hotRes.ok) throw new Error('HTTP ' + hotRes.status);
    
    const hotData = await hotRes.json();
    const rawMovies = hotData.result || [];
    ...
    if (movies.length === 0) {
      return JSON.stringify({ error: 'No hot movies found after filtering trailers' });
    }
    ...
    console.log('[KENG][RoPhim10] Dedicated Phim Hot rail loaded: ' + movies.length + ' movies');
    return JSON.stringify([rail]); // Plain array for rail groups in v6

  } catch (e) {
    console.error('[KENG][RoPhim10] railGroupHot() ERROR: ' + e.message);
