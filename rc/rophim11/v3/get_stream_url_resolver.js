...
async function getStreamUrl(episodeUrl) {
  ...
  try {
    if (/\.m3u8(?:$|\?)/i.test(episodeUrl)) {
      ...
      console.log('[KENG][RoPhim11] Direct m3u8, ads=' + (result.ads === null ? 'null' : (result.ads ? result.ads.length : 0)));
    ...
    if (m3u8Match) {
      ...
      console.log('[KENG][RoPhim11] Stream result: ads=' + (result.ads === null ? 'null' : (result.ads ? result.ads.length : 0)));
      return JSON.stringify(result);
    }

    const contentUrlMatch = html.match(/"contentUrl":"([^"]+\.m3u8[^"]*)"/i);
    if (contentUrlMatch) {
      const streamUrl = contentUrlMatch[1].replace(/\\/g, '');
      const referer = episodeUrl;
      const result = await makeStreamM3U8Result(streamUrl, referer);
      console.log('[KENG][RoPhim11] Stream result: ads=' + (result.ads === null ? 'null' : (result.ads ? result.ads.length : 0)));
      return JSON.stringify(result);
