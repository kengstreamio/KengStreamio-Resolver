...
/**
 * RoPhim1 | Search Resolver v3
 * Contract: searchMovies(baseUrl, keyword, page) -> JSON array
 *
 * The active site now serves search data from JSON API:
 * GET /baseapi/api/v1/movies/search?keyword=...&page=...&limit=32
 */
async function searchMovies(baseUrl, keyword, page = 1) {
  ...
  function isTrailer(apiMovie) {
    const status = String(apiMovie.status || '').toLowerCase();
    const episodeCurrent = String(apiMovie.episode_current || '').toLowerCase();
    const name = String(apiMovie.name || '').toLowerCase();
    const slug = String(apiMovie.slug || '').toLowerCase();
    return status === 'trailer' || episodeCurrent === 'trailer' || name.includes('trailer') || slug.includes('.trailer');
  }

  function transformMovie(apiMovie, rank) {
    ...
    return {
      rank,
      title: apiMovie.name || '',
      title_original: decodeHtml(apiMovie.origin_name || ''),
      poster_url: normalizePoster(apiMovie.poster || apiMovie.thumbnail || ''),
      thumbnail_url: normalizePoster(apiMovie.thumbnail || ''),
  ...
  try {
    ...
    const url = `${BASE_API}/movies/search?page=${currentPage}&limit=32&keyword=${encodeURIComponent(String(keyword).trim())}`;
    console.log(`[KENG][RoPhim1] searchMovies: "${keyword}" (page ${currentPage})`);
    ...
    console.log(`[KENG][RoPhim1] searchMovies: returned ${movies.length} results`);
    return JSON.stringify(movies);
  } catch (e) {
    console.error(`[KENG][RoPhim1] searchMovies ERROR: ${e.message}`);
