...
/**
 * Episodes Resolver v3 — RoPhim1
 * Source: https://api.<host>/api/v1/movies/get-episodes/{slug}
 *
 * Unlike RoPhim9/10, RoPhim1 does not expose /episodes/by-idMovie. The episode
 * list is attached directly to the slug-based movie payload.
 */
async function getEpisodes(filmUrl) {
  ...
  try {
    console.log(`[KENG][RoPhim1] getEpisodes: ${filmUrl}`);
    ...
    const episodesUrl = `${API_ORIGIN}/api/v1/movies/get-episodes/${encodeURIComponent(slug)}`;
    const response = await fetch(episodesUrl, buildFetchOptions(filmUrl));

    if (!response.ok) {
      throw new Error(`Failed to get episodes: ${response.status}`);
    }

    const moviePayload = await response.json();
    const episodeItems = Array.isArray(moviePayload && moviePayload.episodes) ? moviePayload.episodes : [];
    const movieSlug = moviePayload && moviePayload.slug ? moviePayload.slug : slug;
    ...
    for (const item of episodeItems) {
      ...
      const epKey = epNum !== null ? epNum : String(item.name || '');
      ...
      if (item.server && item.id) {
        episode.servers.push({
          server: String(item.server).replace(/#\d+$/, '').replace(/:$/, '').trim(),
          url: `${SITE_BASE}/xem-phim/${movieSlug}.${item.id}`
        });
      }
    }

    const episodes = Array.from(episodeMap.values()).sort((a, b) => a.episode_index - b.episode_index);
    console.log(`[KENG][RoPhim1] getEpisodes: returned ${episodes.length} episodes`);
    return JSON.stringify(episodes);
  } catch (e) {
    console.error(`[KENG][RoPhim1] getEpisodes ERROR: ${e.message}`);
