...
async function railGroupPhimHan(baseUrl) {
  ...
  function logRail(message) {
    console.log('[KENG][opensource-1][railGroupPhimHan] ' + message);
  }
  ...
  function makeRail(movies) {
    return {
      ...
      show_rank: false,
      movies: movies,
      show_cta: { js_method: 'getAllPhimHan' }
    };
  }

  try {
    logRail('start');
    const movies = await fetchItems(baseUrl, '/v1/api/quoc-gia/han-quoc', 1);
...
async function getAllPhimHan(baseUrl, page = 1) {
  function resolveBaseUrl(baseUrl) {
    return (typeof baseUrl === 'string' && /^https?:\/\//i.test(baseUrl))
      ? baseUrl
      : ((typeof location !== 'undefined' && location.origin) ? location.origin : '');
  ...
  try {
    ...
    const movies = await fetchItems(baseUrl, '/v1/api/quoc-gia/han-quoc', page);
