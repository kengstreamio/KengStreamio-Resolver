...
async function railGroupPhimHot(baseUrl) {
  ...
  function logRail(message) {
    console.log('[KENG][opensource-1][railGroupPhimHot] ' + message);
  }
  ...
  function makeRail(movies) {
    return {
      ...
      show_rank: false,
      movies: movies,
      show_cta: { js_method: 'getAllPhimHot' }
    };
  }

  try {
    logRail('start');
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/phim-chieu-rap', 1, 'movie');
...
async function getAllPhimHot(baseUrl, page = 1) {
  function resolveBaseUrl(baseUrl) {
    return (typeof baseUrl === 'string' && /^https?:\/\//i.test(baseUrl))
      ? baseUrl
      : ((typeof location !== 'undefined' && location.origin) ? location.origin : '');
  ...
  try {
    ...
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/phim-chieu-rap', page, 'movie');
