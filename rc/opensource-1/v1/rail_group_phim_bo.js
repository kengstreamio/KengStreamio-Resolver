...
async function railGroupPhimBo(baseUrl) {
  ...
  function logRail(message) {
    console.log('[KENG][opensource-1][railGroupPhimBo] ' + message);
  }
  ...
  function makeRail(movies) {
    return {
      ...
      show_rank: false,
      movies: movies,
      show_cta: { js_method: 'getAllPhimBo' }
    };
  }

  try {
    logRail('start');
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/phim-bo', 1);
...
async function getAllPhimBo(baseUrl, page = 1) {
  ...
  try {
    ...
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/phim-bo', page);
