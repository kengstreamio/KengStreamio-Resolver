...
function imgUrl(url) {
  ...
  if (!text) return '';
  try {
    return new URL(text, 'https://phimimg.com').href;
  } catch (_e) {
    return text;
  }
}
...
function parseGenres(movie) {
  if (!movie || !Array.isArray(movie.category)) return [];
...
function parseCountries(movie) {
  if (!movie || !Array.isArray(movie.country)) return '';
...
function parseActors(movie) {
  const rawActors = Array.isArray(movie && movie.actors)
    ? movie.actors
    : (Array.isArray(movie && movie.actor) ? movie.actor : []);
...
function parseEpisodes(movie, baseUrl, filmSlug) {
  const groups = Array.isArray(movie.episodes) ? movie.episodes : [];
  const mapped = [];
  let episodeCounter = 0;

  groups.forEach((group) => {
    const serverName = cleanText(group.server_name || 'Server').replace(/:$/, '');
    const serverData = Array.isArray(group.server_data) ? group.server_data : [];
    serverData.forEach((entry) => {
      const rawName = cleanText(entry && entry.name ? entry.name : '');
      ...
      item.servers.push({
        server: serverName,
        url: absUrl(baseUrl, entry.link_m3u8 || entry.link_embed || `/phim/${encodeURIComponent(filmSlug)}`)
      });
    });
  });
...
async function getMovieDetail(filmUrl) {
  try {
    if (!filmUrl) {
      throw new Error('Missing filmUrl');
    }

    const url = new URL(filmUrl, resolveBaseUrl(filmUrl));
    const filmSlug = cleanText(url.pathname.split('/').filter(Boolean).pop() || '');
    if (!filmSlug) {
      throw new Error('Invalid filmUrl');
    }

    const apiUrl = new URL(`/phim/${encodeURIComponent(filmSlug)}`, resolveBaseUrl(filmUrl));
    const json = await fetchJson(apiUrl.href);
    const movie = json && json.movie ? json.movie : {};
    const poster = cleanText(movie.poster_url || '');
    const thumb = cleanText(movie.thumb_url || '');
    const episodeCurrent = cleanText(movie.episode_current || '');
    const title = cleanText(movie.name || '');
    const titleOriginal = cleanText(movie.origin_name || '');
    const parts = parseEpisodes(json || {}, resolveBaseUrl(filmUrl), filmSlug);
    ...
    const detail = {
      ...
      duration: cleanText(movie.time || ''),
      rating: cleanText(movie.tmdb && movie.tmdb.vote_average ? String(movie.tmdb.vote_average) : ''),
      ...
      total_episodes: cleanText(movie.episode_total || '') ? Number.parseInt(movie.episode_total, 10) || 0 : parts.reduce((n, part) => n + (Array.isArray(part.episodes) ? part.episodes.length : 0), 0),
      badge_text: cleanText(movie.episode_current || movie.status || ''),
      parts: [
        {
          name: 'Phần 1',
          episodes: parts.length ? parts : [{
            ...
            servers: [{
              server: 'Server 1',
              url: absUrl(resolveBaseUrl(filmUrl), `/phim/${encodeURIComponent(filmSlug)}`)
            }]
          }]
        }
      ]
