...
async function railGroupHoatHinh(baseUrl) {
  ...
  function logRail(message) {
    console.log('[KENG][opensource-1][railGroupHoatHinh] ' + message);
  }
  ...
  function makeRail(movies) {
    return {
      ...
      show_rank: false,
      movies: movies,
      show_cta: { js_method: 'getAllHoatHinh' }
    };
  }

  try {
    logRail('start');
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/hoat-hinh', 1);
...
async function getAllHoatHinh(baseUrl, page = 1) {
  function resolveBaseUrl(baseUrl) {
    return (typeof baseUrl === 'string' && /^https?:\/\//i.test(baseUrl))
      ? baseUrl
      : ((typeof location !== 'undefined' && location.origin) ? location.origin : '');
  ...
  try {
    ...
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/hoat-hinh', page);
