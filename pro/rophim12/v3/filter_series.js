...
// Story 10-13 | RoPhim12 | Filter Phim Bộ — v3
...
//
// LIMITATION: RoPhim12 filter API requires auth. Fallback to /movies/by-type/series (no filter support).
// Returns all series sorted by updatedAt. Filter params ignored.
// TODO: Request public filter API from RoPhim12 team.
//
...
// Genres: index → live category API ID, ordered exactly as /baseapi/api/v1/categories?limit=100 returns.
...
async function filterSeries(baseUrl, 
    ...
    function extractCardImageFromEmbeddedJson(html, slug) {
        const slugNeedle = '\\"slug\\":\\"' + slug + '\\"';
        let idx = html.indexOf(slugNeedle);
        if (idx === -1) {
            const rawNeedle = '"slug":"' + slug + '"';
        ...
        const window = html.slice(idx, idx + 6000);
        // Category cards on RoPhim12 use portrait thumbnails.
        // Prefer `thumbnail` from the embedded JSON, then fall back to `poster`.
        const imageMatch =
            window.match(/\\"thumbnail\\":\\"([^\\"]+)\\"/) ||
            window.match(/"thumbnail":"([^"]+)"/) ||
            window.match(/\\"poster\\":\\"([^\\"]+)\\"/) ||
            window.match(/"poster":"([^"]+)"/);
    ...
    try {
        console.log('[KENG][RoPhim12] filterSeries: page ' + page);
        const url = buildUrl();
        ...
        while ((m = itemRe.exec(html)) !== null) {
            ...
            // Fallback: RoPhim12 embeds the real episode_current inside the Next.js payload.
            ...
            if (!movieData[link].poster) {
                const embeddedPoster = extractCardImageFromEmbeddedJson(html, slug);
                if (embeddedPoster) {
                    movieData[link].poster = embeddedPoster.startsWith('//')
                        ? 'https:' + embeddedPoster
                        : embeddedPoster;
                }
            }

            // Final fallback only when embedded JSON is missing the card image.
            if (!movieData[link].poster) {
        ...
        console.log('[KENG][RoPhim12] filterSeries SUCCESS: ' + finalResults.length + ' items (page ' + page + ')');
        return JSON.stringify(finalResults);

    } catch (e) {
        console.log('[KENG][RoPhim12] filterSeries error: ' + e.message);
