...
async function fetchItems(baseUrl, page = 1) {
  const siteBase = resolveBaseUrl(baseUrl);
  const url = new URL('/api/films/phim-moi-cap-nhat', siteBase);
...
async function railGroupPhimMoiCapNhat(baseUrl) {
  try {
    const movies = await fetchItems(baseUrl, 1);
    return JSON.stringify([{
      ...
      show_rank: false,
      movies,
      show_cta: { js_method: 'getAllPhimMoiCapNhat' }
    }]);
  } catch (e) {
...
async function getAllPhimMoiCapNhat(baseUrl, page = 1) {
  try {
    const movies = await fetchItems(baseUrl, page);
