...
function buildListUrl(baseUrl, section, page) {
  const siteBase = resolveBaseUrl(baseUrl);
  const url = new URL(section, siteBase);
  url.searchParams.set('page', String(Number.parseInt(page || '1', 10) || 1));
  return url.href;
}

function buildFilterUrl(baseUrl, sortVal, countryVal, yearVal, genreVal, page) {
  const siteBase = resolveBaseUrl(baseUrl);
  const yearText = cleanText(yearVal);
  const countrySlug = slugify(countryVal);
  const genreSlug = slugify(genreVal);

  let path = '/api/films/danh-sach/phim-bo';
  if (yearText) {
    path = `/api/films/nam-phat-hanh/${encodeURIComponent(yearText)}`;
  } else if (genreSlug) {
    path = `/api/films/the-loai/${encodeURIComponent(genreSlug)}`;
  } else if (countrySlug) {
    path = `/api/films/quoc-gia/${encodeURIComponent(countrySlug)}`;
  }
...
async function runFilter(baseUrl, sortVal, countryVal, yearVal, genreVal, page) {
  const url = buildFilterUrl(baseUrl, sortVal, countryVal, yearVal, genreVal, page);
  const json = await fetchJson(url);
  const items = Array.isArray(json && json.items) ? json.items : [];
  return items
    .filter((item) => item && !isTrailer(item))
    .map((item) => normalizeMovieItem(baseUrl, item))
...
async function filterSeries() {
  ...
  try {
    const movies = await runFilter(baseUrl, sortVal, countryVal, yearVal, genreVal, page);
