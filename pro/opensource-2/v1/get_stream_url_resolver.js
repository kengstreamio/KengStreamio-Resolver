...
function extractSlug(value) {
  try {
    const url = new URL(cleanText(value), resolveBaseUrl(value));
...
function extractStreamInfo(json) {
  ...
  for (const group of groups) {
    ...
    for (const item of items) {
      const embed = cleanText(item && item.embed || '');
      const m3u8 = cleanText(item && item.m3u8 || '');

      if (embed) {
        return { url: embed, type: 'embed' };
      }
      if (m3u8) {
...
async function getStreamUrl(episodeUrl) {
  try {
    if (!episodeUrl) throw new Error('Missing episodeUrl');

    const direct = cleanText(episodeUrl);

    if (direct.includes('embed.php') || direct.includes('streamc.xyz')) {
      return JSON.stringify({
        type: 'embed',
        url: direct,
        headers: { Referer: originOf(direct) ? originOf(direct) + '/' : '' }
      });
    }
    ...
    const slug = extractSlug(direct);
    if (!slug) throw new Error('Cannot resolve stream url');

    const siteBase = resolveBaseUrl(direct);
    const json = await fetchJson(new URL(`/api/film/${encodeURIComponent(slug)}`, siteBase).href);
    const streamInfo = extractStreamInfo(json);
    if (!streamInfo) throw new Error('No stream link found');

    let streamUrl = streamInfo.url;
    let referer = originOf(streamUrl) ? originOf(streamUrl) + '/' : (siteBase ? siteBase.replace(/\/$/, '') + '/' : '');

    return JSON.stringify({
      type: streamInfo.type === 'm3u8' ? 'embed' : streamInfo.type,
      url: streamUrl,
