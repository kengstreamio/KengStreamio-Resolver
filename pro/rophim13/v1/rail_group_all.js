...
async function railGroupAll(baseUrl) {
  ...
  const THEMES = [
    ...
    { slug: 'de-xuat-cho-ban',                          title: 'Đề Xuất Cho Bạn'                                                     },
    { slug: 'dang-chieu-phat',                          title: 'Đang Chiếu Phát'                                                    },
    { slug: 'chieu_rap',                               title: 'Phim Chiếu Rạp',                               is_cinema: true       },
    { slug: 'phim-chat-luong-cao-va-phu-de-song-ngu',   title: 'Phim Chất Lượng Cao và Phụ Đề Song Ngữ'                            },
    { slug: 'hoat-hinh-chon-loc',                       title: 'Hoạt Hình Chọn Lọc'                                                 },
    { slug: 'phieu-luu-mao-hiem',                       title: 'Phiêu Lưu Mạo Hiểm'                                                 },
    { slug: 'phim-truyen-hinh-trung-quoc-dai-luc',      title: 'Phim Truyền Hình Trung Quốc Đại Lục'                               },
    { slug: 'tinh-yeu-la-nhung-gi-trai-tim-muon',       title: 'Tình Yêu Là Những Gì Trái Tim Muốn'                                },
    { slug: 'co-trang-huyen-ao',                        title: 'Cổ Trang Huyền Ảo'                                                  },
    { slug: 'phim-han-quoc',                            title: 'Phim Hàn Quốc'                                                      },
    { slug: 'thanh-xuan',                               title: 'Thanh Xuân'                                                         },
    { slug: 'phim-chua-lanh-tam-hon',                   title: 'Phim Chữa Lành Tâm Hồn'                                            },
    { slug: 'phim-chuyen-the-tu-tac-pham-van-hoc',      title: 'Phim Chuyển Thể Từ Tác Phẩm Văn Học'                              },
    { slug: 'phim-4k',                                  title: 'Phim 4K'                                                            },
    { slug: 'phim-cong-so',                             title: 'Phim Công Sở'                                                       },
    { slug: 'hinh-su-toi-pham-han-quoc',                title: 'Hình Sự Tội Phạm Hàn Quốc'                                        },
    { slug: 'phim-co-trang-huyen-huyen-khong-the-bo-lo', title: 'Phim Cổ Trang Huyền Huyễn Không Thể Bỏ Lỡ'                      },
    { slug: 'dien-anh-au-my',                           title: 'Điện Ảnh Âu Mỹ'                                                    },
  ];
  ...
  for (let i = 0; i < THEMES.length; i++) {
    ...
    rails.push({
      ...
      show_rank: false,
      movies: movies,
      show_cta: { js_method: `getAll_${theme.slug}` },
    });
  }
...
async function getAll_phim_hot(page = 1) {
  const res = await fetch(`${_ROPHIM13_API}/movies/hot`, _rophim13FetchOpts());
  ...
  return JSON.stringify(
    movies
      ...
      .map(m => _rophim13TransformMovie(m, 'https://onflix.lol'))
...
async function getAll_de_xuat_cho_ban(page = 1) {
  ...
  return JSON.stringify(movies.filter(m => m.title && m.url));
}
...
async function getAll_chieu_rap(page = 1) {
  const url = `${_ROPHIM13_API}/movies?type=chieu_rap&sort=newest&page=${page}&limit=12`;
  const res = await fetch(url, _rophim13FetchOpts());
  if (!res.ok) return JSON.stringify([]);
  const data = await res.json();
  const movies = (data.data || []).map(m => _rophim13TransformMovie(m, 'https://onflix.lol'));
  return JSON.stringify(movies.filter(m => m.title && m.url));
}
