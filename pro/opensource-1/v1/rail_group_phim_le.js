...
async function railGroupPhimLe(baseUrl) {
  ...
  function logRail(message) {
    console.log('[KENG][opensource-1][railGroupPhimLe] ' + message);
  }
  ...
  try {
    logRail('start');
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/phim-le', 1);
...
async function getAllPhimLe(baseUrl, page = 1) {
  ...
  try {
    ...
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/phim-le', page);
