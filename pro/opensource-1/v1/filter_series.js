...
function splitArgs(rawArgs) {
  ...
  return { baseUrl: resolveBaseUrl(baseUrl), args: args };
}
...
function isTrailer(text) {
  return /trailer/i.test(cleanText(text));
}
...
function normalizeMovieItem(baseUrl, item, mediaType) {
  ...
  const titleOriginal = cleanText(item.origin_name || '');
  ...
  return {
    ...
    media_type: mediaType || inferMediaType(item),
...
function buildFilterEndpoint(baseUrl, section, sortVal, countryVal, yearVal, genreVal, page) {
  const pageNum = Number.parseInt(page || '1', 10) || 1;
  const siteBase = resolveBaseUrl(baseUrl);
  const url = new URL(`/v1/api/${section}`, siteBase);
  const sortType = /cũ|asc/i.test(cleanText(sortVal)) ? 'asc' : 'desc';
  const genreSlug = slugify(genreVal);
  const countrySlug = slugify(countryVal);
  const yearText = cleanText(yearVal);

  if (yearText) {
    url.pathname = `/v1/api/nam/${encodeURIComponent(yearText)}`;
  } else if (genreSlug) {
    url.pathname = `/v1/api/the-loai/${encodeURIComponent(genreSlug)}`;
  } else if (countrySlug) {
    url.pathname = `/v1/api/quoc-gia/${encodeURIComponent(countrySlug)}`;
  }

  url.searchParams.set('page', String(pageNum));
  url.searchParams.set('limit', '20');
  url.searchParams.set('sort_field', 'modified.time');
  url.searchParams.set('sort_type', sortType);

  if (genreSlug) {
    url.searchParams.set('category', genreSlug);
  }
  if (countrySlug) {
    url.searchParams.set('country', countrySlug);
  }
  if (yearText) {
    url.searchParams.set('year', yearText);
  }

  return url.href;
}

async function runFilter(baseUrl, section, mediaType, sortVal, countryVal, yearVal, genreVal, page) {
  const url = buildFilterEndpoint(baseUrl, section, sortVal, countryVal, yearVal, genreVal, page);
  const json = await fetchJson(url);
  const items = Array.isArray(json && json.data && json.data.items) ? json.data.items : [];
  return items
    .filter((item) => item && !isTrailer(item.episode_current || item.name || item.origin_name || ''))
    .map((item) => normalizeMovieItem(baseUrl, item, mediaType))
    .filter((movie) => movie.title && movie.url);
}

async function filterSeries() {
  const { baseUrl, args } = splitArgs(arguments);
  const [sortIdx, sortVal, countryId, countryIdx, countryVal, yearId, yearIdx, yearVal, genreId, genreIdx, genreVal, page] = args;

  try {
    const movies = await runFilter(baseUrl, 'danh-sach/phim-bo', 'series', sortVal, countryVal, yearVal, genreVal, page);
