...
async function railGroupTvShows(baseUrl) {
  ...
  function logRail(message) {
    console.log('[KENG][opensource-1][railGroupTvShows] ' + message);
  }
  ...
  function makeRail(movies) {
    return {
      ...
      show_rank: false,
      movies: movies,
      show_cta: { js_method: 'getAllTvShows' }
    };
  }

  try {
    logRail('start');
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/tv-shows', 1);
...
async function getAllTvShows(baseUrl, page = 1) {
  function resolveBaseUrl(baseUrl) {
    return (typeof baseUrl === 'string' && /^https?:\/\//i.test(baseUrl))
      ? baseUrl
      : ((typeof location !== 'undefined' && location.origin) ? location.origin : '');
  ...
  try {
    ...
    const movies = await fetchItems(baseUrl, '/v1/api/danh-sach/tv-shows', page);
