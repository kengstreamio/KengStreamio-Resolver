...
async function railGroupPhimUsUk(baseUrl) {
  ...
  function logRail(message) {
    console.log('[KENG][opensource-1][railGroupPhimUsUk] ' + message);
  }
  ...
  function makeRail(movies) {
    return {
      ...
      show_rank: false,
      movies: movies,
      show_cta: { js_method: 'getAllPhimUsUk' }
    };
  }

  try {
    logRail('start');
    const movies = await fetchItems(baseUrl, '/v1/api/quoc-gia/au-my', 1);
...
async function getAllPhimUsUk(baseUrl, page = 1) {
  function resolveBaseUrl(baseUrl) {
    return (typeof baseUrl === 'string' && /^https?:\/\//i.test(baseUrl))
      ? baseUrl
      : ((typeof location !== 'undefined' && location.origin) ? location.origin : '');
  ...
  try {
    ...
    const movies = await fetchItems(baseUrl, '/v1/api/quoc-gia/au-my', page);
