...
// Categories: Filter Movies by Genre
// Function: filterMovies(baseUrl, sortIdx, sortVal, [filterGroup]*, page) -> JSON string
// Filter groups come as triples: [id, idx, val] dynamically from app config.
// For anime-2: 1 sort group + 2 filter groups (year, genre).
// hhpanda.st has NO year-based listing — year filter is a no-op.
// Genre maps to /the-loai/{slug}. Fallback to homepage when no genre selected.
// v6.1 contract: dynamic params, baseUrl as first arg.

async function filterMovies(...allArgs) {
  // Extract baseUrl, sortIdx, sortVal
  const baseUrl = (allArgs[0] || '').replace(/\/+$/, '');
  ...
  // Find genre filter group (year is ignored — site has no year-based listing)
  let genreIdx = '-1';
  ...
  // anime-2 genre slugs (same as filter_series)
  const GENRE_SLUGS = [
    'tu-tien',        // 0: Tu Tiên
    'kiem-hiep',      // 1: Kiếm Hiệp
    'co-trang',       // 2: Cổ Trang
    'huyen-huyen',    // 3: Huyền Huyễn
    'khoa-huyen',     // 4: Khoa Huyễn
    'ky-ao',          // 5: Kỳ Ảo
    'huyen-nghi',     // 6: Huyền Nghi
    'canh-ky',        // 7: Canh Kỹ
    'da-su',          // 8: Dã Sử
    'do-thi',         // 9: Đô Thị
    'dong-nhan'       // 10: Đồng Nhan
  ];
  ...
  // Fallback: homepage listing when no genre selected
  let url;
  if (slug) {
    url = page <= 1
      ? `${baseUrl}/the-loai/${slug}`
      : `${baseUrl}/the-loai/${slug}/page/${page}`;
  } else {
    url = page <= 1
      ? baseUrl
      : `${baseUrl}/page/${page}`;
  ...
  try {
    ...
    const movies = parseGenericList(html, baseUrl);
...
function parseGenericList(html, baseUrl) {
  ...
  const articles = doc.querySelectorAll('article.thumb.grid-item, article.halim-item, article');
  const movies = [];
