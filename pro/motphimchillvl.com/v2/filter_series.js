...
// URL: /?filter[type]=series&filter[sort]=view&filter[region]=ID&filter[year]=YYYY&filter[category]=ID&page=N on the active provider origin
//
...
async function filterSeries(
    ...
    function parsePage(html) {
        ...
        for (let i = 1; i < liParts.length; i++) {
            ...
            const epM = label.match(/Tập\s*(\d+)/i);
