...
async function getStreamUrl(episodeUrl) {
    ...
    try {
        ...
        if (m3u8Matches && m3u8Matches.length > 0) {
            ...
            console.log('[KENG][RoPhim9] Found m3u8 in watch page: ' + streamUrl);
            const referer = SITE_BASE + '/';
            const result = await makeStreamM3U8Result(streamUrl, referer);
            console.log('[KENG][RoPhim9] Stream result: ads=' + (result.ads === null ? 'null' : (result.ads ? result.ads.length : 0)));
            return JSON.stringify(result);
        }
        ...
        if (iframeM) {
            ...
            if (embedMatches && embedMatches.length > 0) {
                ...
                console.log('[KENG][RoPhim9] Found m3u8 in embed: ' + streamUrl);
        ...
        if (epIdM) {
            ...
            if (embedMatches && embedMatches.length > 0) {
                const streamUrl = embedMatches[0].replace(/\\/g, '');
                console.log('[KENG][RoPhim9] Found m3u8 via direct embed: ' + streamUrl);
        ...
        console.log('[KENG][RoPhim9] getStreamUrl error: ' + e.message);
